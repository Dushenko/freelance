...
$(document).ready(function () {
  if ($(window).width() <= 1220) {
    $('.filter-section').append($('.filter'));
  }

  var slideCount = $('.slideCount');
  var slickSlide = $('.main-slider');
  ...
  $('.product-slider__image').slick({
    arrows: false,
    fade: true,
    asNavFor: '.product-slider__feed'
  });
  $('.product-slider__feed').slick({
    slidesToShow: 3,
    asNavFor: '.product-slider__image',
    variableWidth: true
  });
  $('.busket-counter').click(function (e) {
    var myelem = $(this.children[1]);
    if ($(e.target).hasClass('busket-counter__plus')) myelem.html(+myelem.html() + 1);
    if ($(e.target).hasClass('busket-counter__minus') && myelem.html() > 1) myelem.html(+myelem.html() - 1);
  });
  $('.open-modal').click(function () {
    $($(this).attr('data-modal')).addClass('modal-open');
  });
  $('.modal').click(function (e) {
    if ($(e.target).hasClass('modal') || $(e.target).hasClass('modal__exit')) {
      $(this).removeClass('modal-open');
    }
  });
  var timer = '';
  $('.catalog__slide').click(function (e) {
    var cat = $(this).parent().parent();
    var height = $(this).parent().height();

    if (cat.hasClass('catalog-active')) {
      clearTimeout(timer);
      cat.removeClass('catalog-active').css({
        maxHeight: '60px',
        overflow: 'hidden'
      });
    } else {
      cat.addClass('catalog-active').css('max-height', height);
      timer = setTimeout(function () {
        cat.css('overflow', 'visible');
      }, 700);
    }
  });
  $('.phone-search').click(function () {
    $('.header .basket').hide();
    $('.phone-logo').hide();
    $('.header__phone-input').show().focus();
  });
  $('.header__phone-input').keydown(function (e) {
    if (e.keyCode === 13) {
      $(this).hide();
      $('.header .basket').show();
      $('.phone-logo').show();
  ...
  var isSubOpen = false;
  $('.catalog__link_has-children').click(function () {
    $('.catalog__sub-title').html($(this).html());
    $(this).find('.catalog__submenu').addClass('catalog__submenu_open');
    isSubOpen = true;
  });
  $('.catalog__phone-close').click(function () {
    if (isSubOpen) {
      $('.catalog__sub-title').html('');
      $('.catalog__submenu_open').removeClass('catalog__submenu_open');
      isSubOpen = false;
    } else $('.catalog-menu_open').removeClass('catalog-menu_open');
  });
  $('.burger-wrp').click(function () {
    $('.catalog-menu').addClass('catalog-menu_open');
  });
});